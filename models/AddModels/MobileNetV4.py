...
__all__ = ['MobileNetV4ConvTiny', 'MobileNetV4ConvLarge', 'MobileNetV4ConvSmall', 'MobileNetV4ConvMedium', 'MobileNetV4HybridMedium', 'MobileNetV4HybridLarge']
...
# ===================================================================================
# 在 MNV4ConvSmall_BLOCK_SPECS 字典的【上方】或【下方】添加这个新字典
# ===================================================================================

MNV4ConvTiny_BLOCK_SPECS = {
    "conv0": {
        "block_name": "convbn",
        "num_blocks": 1,
        "block_specs": [
            # [inp, oup, kernel, stride]
            [3, 24, 3, 2] # 通道数 32 -> 24
        ]
    },
    "layer1": { # P1/4
    ...
    "layer2": { # P2/8
        ...
        "num_blocks": 2,
        "block_specs": [
            [24, 48, 3, 2], # 通道数 32/96 -> 24/48
            [48, 40, 1, 1]  # 通道数 64 -> 40
        ]
    },
    "layer3": { # P3/16
        "block_name": "uib",
        "num_blocks": 3, # 深度从 6 -> 3
        "block_specs": [
            # [inp, oup, start_k, middle_k, middle_downsample, stride, expand_ratio]
            [40, 48, 5, 5, True, 2, 3], # 通道数 64/96 -> 40/48
            [48, 48, 0, 3, True, 1, 2], # 通道数 96 -> 48
            [48, 48, 0, 3, True, 1, 2], # 通道数 96 -> 48
        ]
    },
    "layer4": { # P4/32
        ...
        "num_blocks": 3, # 深度从 6 -> 3
        "block_specs": [
            [48, 96, 3, 3, True, 2, 6],  # 通道数 96/128 -> 48/96
            [96, 96, 5, 5, True, 1, 4],  # 通道数 128 -> 96
            [96, 96, 0, 5, True, 1, 4],  # 通道数 128 -> 96
        ]
    },
    "layer5": { 
        # Layer5 对于检测任务的 Backbone 来说不是必需的，我们将其移除
        "block_name": None 
    }
}
...
MODEL_SPECS = {
    "MobileNetV4ConvTiny": MNV4ConvTiny_BLOCK_SPECS,
...
def MobileNetV4ConvSmall():
    ...
    return model
 # 在文件末尾，添加一个新的函数来创建 Tiny 模型
def MobileNetV4ConvTiny():
    model = MobileNetV4('MobileNetV4ConvTiny')
    return model
